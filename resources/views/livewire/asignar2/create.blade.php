    ...
    <form wire:submit.prevent="save" class="needs-validation" novalidate>
        <div class="modal-body">
            ...
            <div class="row">
                <div class="col-lg">
                    <div class="mb-1">
                        <label class="form-label" for="select2-form">Areas</label>
                        <select wire:model.defer="area_id" class="form-select @error('area_id') is-invalid @enderror">
                            <option value="">--- Seleccionar ---</option>
                            @foreach ($areas as $d)
                                    <option value="{{ $d->id_area }}">{{ $d->area }}</option>
                                @endforeach
                        </select>
                            @error('area_id')
                                <small class="text-danger">{{ $message }}</small>
